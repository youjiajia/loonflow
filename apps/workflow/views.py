...
class WorkflowView(LoonBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        view_permission_check = request_data_dict.get(
            'view_permission_check', 1)
...
class WorkflowUserAdminView(LoonBaseView):
    def get(self, request, *args, **kwargs):
        ...
        """
        username = request.META.get('HTTP_USERNAME')
        flag, result = workflow_base_service_ins.get_workflow_manage_list(
            username)
        if flag is False:
...
class WorkflowTransitionView(LoonBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        transition_type_id = int(
            request_data_dict.get('transition_type_id', 0))
        ...
        condition_expression = request_data_dict.get(
...
class WorkflowStateDetailView(LoonBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        flag, state_info_dict = workflow_state_service_ins.get_restful_state_info_by_id(
            state_id)
        if flag is not False:
...
class WorkflowRunScriptView(LoonBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if file_obj:  # 处理附件上传到方法
            ...
            script_file_name = "workflow_script/{}.py".format(
                str(uuid.uuid1()))
...
class WorkflowRunScriptDetailView(LoonBaseView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        """
        run_script_id = kwargs.get('run_script_id')
        result, msg = workflow_run_script_service_ins.del_run_script(
            run_script_id)
...
class WorkflowCustomNoticeView(LoonBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if result is not False:
            data = dict(
                value=result, per_page=msg['per_page'], page=msg['page'], total=msg['total'])
            code, msg, = 0, ''
        else:
            code, data = -1, ''
        return api_response(code, msg, data)
    ...
    def post(self, request, *args, **kwargs):
        ...
        result, msg = workflow_custom_notice_service_ins.add_custom_notice(name, description, hook_url, hook_token,
                                                                           creator)
        if result is not False:
            data = {}
            code, msg, = 0, ''
