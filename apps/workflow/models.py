...
class State(BaseModel):
    ...
    order_id = models.IntegerField(
        '状态顺序', default=0, help_text='用于工单步骤接口时，step上状态的顺序(因为存在网状情况，所以需要人为设定顺序),值越小越靠前')
    ...
    enable_retreat = models.BooleanField(
        '允许撤回', default=False, help_text='开启后允许工单创建人在此状态直接撤回工单到初始状态')

    remember_last_man_enable = models.BooleanField('记忆最后处理人', default=False,
                                                   help_text='开启后，到达此状态时会先检查之前是否有人在此状态处理过，如果有则处理人为最后一次处理的人')
    participant_type_id = models.IntegerField('参与者类型id', default=1, blank=True,
                                              help_text='0.无处理人,1.个人,2.多人,3.部门,4.角色,5.变量(支持工单创建人,创建人的leader),'
                                                        '6.脚本,7.工单的字段内容(如表单中的"测试负责人"，需要为用户名或者逗号隔开的多个用户名),'
                                                        '8.父工单的字段内容。 初始状态请选择类型5，参与人填creator')
...
class Workflow(BaseModel):
    ...
    notices = models.CharField('通知', default='', blank=True,
                               max_length=50, help_text='CustomNotice中的id.逗号隔开多个通知方式')
    view_permission_check = models.BooleanField(
        '查看权限校验', default=True, help_text='开启后，只允许工单的关联人(创建人、曾经的处理人)有权限查看工单')
    limit_expression = models.CharField('限制表达式', max_length=1000, default='{}', blank=True,
                                        help_text='限制周期({"period":24} 24小时), 限制次数({"count":1}在限制周期内只允许提交1次)'
                                                  ', 限制级别({"level":1} 针对(1单个用户 2全局)限制周期限制次数,默认特定用户);'
                                                  '允许特定人员提交({"allow_persons":"zhangsan,lisi"}只允许张三提交工单,'
                                                  '{"allow_depts":"1,2"}只允许部门id为1和2的用户提交工单，'
                                                  '{"allow_roles":"1,2"}只允许角色id为1和2的用户提交工单)')
    display_form_str = models.CharField('展现表单字段', max_length=10000, default='[]', blank=True,
                                        help_text='默认"[]"，用于用户只有对应工单查看权限时显示哪些字段,field_key的list的json,'
                                                  '如["days","sn"],内置特殊字段participant_info.participant_name:'
                                                  '当前处理人信息(部门名称、角色名称)，state.state_name:'
                                                  '当前状态的状态名,workflow.workflow_name:工作流名称')
    ...
    content_template = models.CharField('内容模板', max_length=1000, default='标题:{title}, 创建时间:{gmt_created}', null=True,
                                        blank=True, help_text='工单字段的值可以作为参数写到模板中，格式如：标题:{title}, 创建时间:{gmt_created}')
    is_valid = models.BooleanField('是否有效, 即所有的工作环节是否关联成功', default=False)
    States = models.ManyToManyField(State)
    end_operator = models.CharField('终止工单操作人', default='', blank=True, max_length=1000,
                                    help_text='可以为空(无处理人的情况，如结束状态)、username\多个username(以,隔开)'
                                              '\部门id\角色id\变量(creator,creator_tl)\脚本记录的id等，包含子工作流的需要设置处理人为loonrobot')
...
class Transition(BaseModel):
    ...
    transition_type_id = models.IntegerField(
        '流转类型', default=1, help_text='1.常规流转，2.定时器流转,需要设置定时器时间')
    timer = models.IntegerField(
        '定时器(单位秒)', default=0, help_text='流转类型设置为定时器流转时生效,单位秒。处于源状态X秒后如果状态都没有过变化则自动流转到目标状态')
    source_state_id = models.IntegerField('源状态id')
    destination_state_id = models.IntegerField('目的状态id')
    condition_expression = models.CharField('条件表达式', max_length=1000, default='[]',
                                            help_text='流转条件表达式，根据表达式中的条件来确定流转的下个状态，'
                                                      '格式为[{"expression":"{days} > 3 and {days}<10", '
                                                      '"target_state_id":11}] 其中{}用于填充工单的字段key,运算时会换算成实际的值，'
                                                      '当符合条件下个状态将变为target_state_id中的值,表达式只支持简单的运算或datetime/time运算.'
                                                      'loonflow会以首次匹配成功的条件为准，所以多个条件不要有冲突')
    attribute_type_id = models.IntegerField(
        '属性类型', default=1, help_text='属性类型，1.同意，2.拒绝，3.其他')
    field_require_check = models.BooleanField('是否校验必填项', default=True,
                                              help_text='默认在用户点击操作的时候需要校验工单表单的必填项,如果设置为否则不检查。用于如"退回"属性的操作，不需要填写表单内容')
    alert_enable = models.BooleanField('点击弹窗提示', default=False)
    alert_text = models.CharField(
        '弹窗内容', max_length=100, default='', blank=True)
    level = models.IntegerField('流转顺序，如果流转类型为3，则使用流转顺序流转', default=1, blank=True)
...
class CustomField(BaseModel):
    ...
    field_type_id = models.IntegerField('类型',
                                        help_text='5.字符串，10.整形，15.浮点型，20.布尔，25.日期，30.日期时间，35.单选框，40.多选框，'
                                                  ...
                                                  '80.附件(只保存路径，多个使用逗号隔开) 85 富文本 90 国家地区')
    field_key = models.CharField(
        '字段标识', max_length=50, help_text='字段类型请尽量特殊，避免与系统中关键字冲突')
    ...
    field_template = models.TextField(
        '文本域模板', default='', blank=True, help_text='文本域类型字段前端显示时可以将此内容作为字段的placeholder')
    boolean_field_display = models.CharField('布尔类型显示名', max_length=100, default='{}', blank=True,
                                             help_text='当为布尔类型时候，可以支持自定义显示形式。{"1":"是","0":"否"}或'
                                                       '{"1":"需要","0":"不需要"}，注意数字也需要引号')
    field_choice = models.CharField('radio、checkbox、select的选项', max_length=1000, default='{}', blank=True,
                                    help_text='radio,checkbox,select,multiselect类型可供选择的选项，格式为json如:'
                                              '{"1":"中国", "2":"美国"},注意数字也需要引号')
...
class WorkflowScript(BaseModel):
    ...
    """
    name = models.CharField('名称', max_length=50)
    saved_name = models.FileField('存储的文件名', upload_to=upload_workflow_script,
    ...
    is_active = models.BooleanField(
        '可用', default=True, help_text='此处可用时，才允许实际执行')
...
class CustomNotice(BaseModel):
    ...
    hook_url = models.CharField(
        'hook url', max_length=100, null=True, blank=True)
    hook_token = models.CharField(
        'hook token', max_length=100, null=True, blank=True)
